...
def backward_softmax(x, grad_outputs):
    ...
    # *** END CODE HERE ***
    z = forward_softmax(x)
    return (-np.outer(z, z)+np.diag(z)).dot(grad_outputs)
    # *** END CODE HERE ***
...
def backward_relu(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    #What the math says happening
    #grad_inputs = np.diag(x>0)
    #grad_outputs[x<=0]=0
    #return np.dot(grad_inputs, grad_outputs)
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    input_channels, width, height = data.shape

    grad_weights = np.zeros(conv_W.shape)
    grad_data = np.zeros(data.shape)
    grad_bias = np.zeros(conv_b.shape)
    ...
    for channel in range(input_channels):
        for x in range(0, input_width, pool_width):
            for y in range(0, input_height, pool_height):
                i=x//pool_width
                j=y//pool_height
                bestxy = np.unravel_index(data[channel, x:(x+pool_width), y:(y+pool_height)].argmax(), (pool_width, pool_height))
                grad_data[channel, x:(x+pool_width), y:(y+pool_height)][bestxy]+=output_grad[channel, i, j]
    return grad_data
    # *** END CODE HERE ***
...
def backward_linear(weights, bias, data, output_grad):
    ...
    grad_inputs=weights.dot(output_grad)
...
def backward_prop(data, labels, params):
    ...
    #grad_logits needs to be reshaped
